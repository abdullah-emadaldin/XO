...
namespace ReposatoryPatternWithUOW.EF.Reposatories
{
    public class UserReposatory:BaseRepository<User>,IUserReposatory
    {
        ...
        {
            if (signupDto is null || await context.Users.AnyAsync(x => x.Email == signupDto.Email) || await context.Users.AnyAsync(x => x.UserName == signupDto.UserName))
                return false;
            try
            {
...

        {
            ...
            var expirationOfJWT = DateTime.Now.AddYears(100);
            ...
            //    await Console.Out.WriteLineAsync(user.IdentityTokenVerification.UserId.ToString());

            //    context.IdentityTokenVerifications.Remove(user.IdentityTokenVerification!);
            //    await context.SaveChangesAsync();
            //}
            context.IdentityTokenVerifications.Where(x => x.UserId == user.Id).ExecuteDelete();

            var t = TokenHandler.GenerateToken(user, expirationOfJWT, options);
            Console.ForegroundColor = ConsoleColor.Yellow;
            await Console.Out.WriteLineAsync(t);
            user.IdentityTokenVerification = new() { ExpiresAt = expirationOfJWT, Token = t };
            context.Attach(user);
        ...
        public async Task<SendCodeResult> SendVerficationCodeAsync(string email, bool? isForResetingPassword = false)
        {
            ...
            if (isForResetingPassword is null or false)
            {
                if (user.EmailConfirmed)
                    return new() { Success = false };
                body = $@"Dear <span style='text-transform:Capitalize'>{user.UserName}</span>,<br><br> 
        ...
        {
            var result = context.IdentityTokenVerifications.Where(x=> x.UserId == id).ExecuteDelete();
